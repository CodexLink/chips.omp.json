...
| \_TRANSIENT                                            | bool   | **False** _($false)_ | Completely hides the transient segment that contains previous command's return code and execution time when the value is set to `True`. This ignores the value of `SEGMENT_DISABLE_TRANSIENT_RECENT_EXEC_TIME`.                                                                                                                                |
...
| \_TRANSIENT_RECENT_EXEC_TIME                           | bool   | **False** _($false)_ | Hides the `execution time` of the previous command executed from the transient prompt.                                                                                                                                                                                                                                                         |
...
**Also please note**, the screenshots provided here has the theme version of commit [91cc394](https://github.com/CodexLink/chips.omp.json/commit/91cc394f1bcca0fa0103b6fa7156edd83ae586b6), before changes were done from the git branch indicator refactorization. Re-screenshot will be done sooner. Instead of parentheses enclosing branch state, they are now just divided from the branch name to make count clearer from the icon.
